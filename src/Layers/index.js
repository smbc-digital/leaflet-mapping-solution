...
const layersFeatureInfoPopup = async (e, layersWithPopup, map, noPopup) => {
  ...
  Leaflet.popup({ keepInView: false, autoPan: false }) //change both to true to enable popup-snapback. not sure of the difference caused by one but not the other being true.
    .setLatLng(e.latlng)
    .setContent(content)
    .openOn(map)
}
